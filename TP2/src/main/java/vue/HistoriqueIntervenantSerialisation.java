...
public class HistoriqueIntervenantSerialisation extends Serialisation {
    ...
    public void appliquer(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        List<Intervention> liste = (List<Intervention>)request.getAttribute("historiqueIntervenant");
        if (liste != null)
        {
            for (Intervention intervention : liste)
            {
                if (intervention.getDate_fin()!= null){
                    ...
                    jsonIntervention.addProperty("eleveNom", intervention.getEleve().getNom());
                    jsonIntervention.addProperty("elevePrenom", intervention.getEleve().getPrenom());
